...
import { Briefcase, Lock, Mail, MailPlus, RotateCw, Send, Trash2 } from "lucide-react";
...
type InvitationSectionProps = {
   ...
   * directly; none locks the form. The server enforces this either way — what is
   * rendered only ever reflects the decision, never makes it.
   */
  roles: InviteRole[];
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <MailPlus className="text-primary-ink size-4" aria-hidden />
          Invitations
        </CardTitle>
        <CardDescription>
          Enter someone&apos;s email address and pick what they should become. We&apos;ll send them a link to
          register, and they join with that role automatically. Each invitation works once, for that address
          only, and expires after {INVITE_TTL_DAYS} days.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {roles.length > 1 && (
                <Select value={role} onValueChange={(value) => setRole(value as InviteRole)}>
                  <SelectTrigger className="w-44" aria-label="Role this invitation grants">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {roles.map((option) => (
              ...
              <Select value={jobRoleId} onValueChange={setJobRoleId}>
                <SelectTrigger className="w-48" aria-label="Job title this invitation assigns">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Button type="submit" disabled={!email.trim() || busy === "invite"} loading={busy === "invite"}>
        ...
        {invitations.length === 0 ? (
          <EmptyState
            ...
            description={
              canIssue ? "Invite your first person above." : "Invitations you send will appear here."
            }
            inset={false}
          />
        ) : (
